...
def get_bin_table(threshold):
    ...
    table = []
    for i in range(256):
        rate = 0.65  # 在threshold的适当范围内进行处理
...
def cut_noise(image):
    ...
    #     for x in range(rows):
    #         image.putpixel((x, y), 255)
    image.save("D:/picture/tiff" + '/'+'t'+str(1)+'.tif')
...
def OCR_lmj(img_path):
    ...
    imgry=imgry.save(img_path)
    imgry = Image.open(img_path)
...
# OCR_lmj('verify_image.png')
# main()
file_name = 'D:/picture/'+str(1)+'.jpg'
OCR_lmj(file_name)